...
def conv_parameters(conv_dim, kernel_size=KS, stride=STR, padding=PAD, dilation=DIL):
...
def conv_output_size(
    in_shape, out_channels, kernel_size=KS, stride=STR, padding=PAD, dilation=DIL
):
...
def conv_input_size(
    out_shape, in_channels, kernel_size=KS, stride=STR, padding=PAD, dilation=DIL
):
...
class Decoder(nn.Module):
    def __init__(
        ...
        fc_in = nn.Linear(in_features=latent_dim, out_features=inner_dim)
        self.layers.append(fc_in)

        for i in range(self.n_layers - 2):
            fc = nn.Linear(in_features=inner_dim + add_dim, out_features=inner_dim)
            self.layers.append(fc)

        fc_out = nn.Linear(in_features=inner_dim + add_dim, out_features=data_dim)
...
class Vae(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = Encoder(
            latent_dim=latent_dim, data_dim=data_dim, inner_dim=inner_dim
        )
...
class EncoderConv(nn.Module):
    def __init__(self, latent_dim, img_shape):
        ...
        self.in_features = functools.reduce((lambda x, y: x * y), self.out_shape2)
        self.fc11 = nn.Linear(
...
class DecoderConv(nn.Module):
    def __init__(self, latent_dim, img_shape, with_sigmoid, riem_log_exp=False):
        ...
        self.convt2 = self.nn_conv_transpose(
            ...
            out_channels=self.convt2_out_channels,
            kernel_size=KS,
            padding=PAD,
            stride=STR,
        )
        ...
        self.out_features = functools.reduce((lambda x, y: x * y), self.in_shape1)
        self.fc2 = nn.Linear(
...
class VaeConv(nn.Module):
    ...
    def __init__(self, latent_dim, img_shape, with_sigmoid, riem_log_exp=False):
        ...
        self.encoder = EncoderConv(latent_dim=self.latent_dim, img_shape=self.img_shape)
...
class EncoderConvPlus(nn.Module):
    ...
    def __init__(self, latent_dim, img_shape, n_blocks=4):
        ...
        self.fc1 = nn.Linear(in_features=self.fcs_infeatures, out_features=latent_dim)

        self.fc2 = nn.Linear(in_features=self.fcs_infeatures, out_features=latent_dim)
...
class DecoderConvPlus(nn.Module):
    ...
    def __init__(self, latent_dim, img_shape, with_sigmoid, n_blocks=3):
        ...
        self.l0 = nn.Linear(in_features=latent_dim, out_features=self.fcs_infeatures)
...
class DecoderConvOrig(nn.Module):
    ...
    def block(self, block_id, in_shape, channels_fact, scale_factor=2, pad=1):
        ...
        bn = nn.BatchNorm2d(conv.out_channels, 1.0e-3)
    ...
    def __init__(self, latent_dim, img_shape, with_sigmoid, in_shape=None, n_blocks=4):
...
class VaeConvOrig(nn.Module):
    def __init__(self, latent_dim, img_shape, with_sigmoid, n_blocks=5):
        ...
        self.encoder = EncoderConvOrig(
            latent_dim=self.latent_dim, img_shape=self.img_shape, n_blocks=self.n_blocks
        )
...
class Discriminator(nn.Module):
    ...
    def __init__(self, latent_dim, img_shape):
        ...
        self.dis1_out_shape = self.dis_conv_output_size(
            in_shape=self.img_shape, out_channels=self.dis1.out_channels
        )
        ...
        self.dis2_out_shape = self.dis_conv_output_size(
            in_shape=self.dis1_out_shape, out_channels=self.dis2.out_channels
        )
        ...
        self.dis3_out_shape = self.dis_conv_output_size(
            in_shape=self.dis2_out_shape, out_channels=self.dis3.out_channels
        )
        ...
        self.dis4_out_shape = self.dis_conv_output_size(
            in_shape=self.dis3_out_shape, out_channels=self.dis4.out_channels
        )
