    ...
    def _train_iteration(self):
        ...
        for batch_idx, batch_data in enumerate(self.train_loader):
            ...
            if "kullbackleibler_circle" in train_params["regularizations"]:
                loss_regularization = losses.kullback_leibler_circle(mu, logvar)
                # Fill gradients on encoder only
                loss_regularization.backward()

            if "on_circle" in train_params["regularizations"]:
                loss_regularization = losses.on_circle(mu, logvar)
                # Fill gradients on encoder only
                loss_regularization.backward()
                ...
                if batch_idx == n_batches - 1:
                    ...
                    if VISDOM:
                        ...
                        recon_win = vis.image(
                            ...
                            win=recon_win,
                            opts=dict(
                                title="Val Epoch {}: Reconstruction".format(epoch),
                                height=height,
                                width=width,
                            ),
                        )
    ...
    if WITH_RAY:
        ...
        search_space = {
            ...
            "lambda_regu": hp.loguniform(
                "lambda_regu", low=np.log(0.001), high=np.log(4.0)
            ),
            "lambda_adv": hp.loguniform(
                "lambda_adv", low=np.log(0.001), high=np.log(4.0)
            ),
        }
