...
def visualize(dataset, reconstruction,
              crds1, crds2,
              metadata1, metadata2,
              iframe=0, markersize=24,
              cmap1=None, cmap2=None,
...
def show_latentspace(mus, metadata,
    ...
    if do_pca:
        ...
        if(label1):
            plt.ylabel(label1)
        plt.scatter(np.arange(metadata.shape[0]),metadata[:,1], c=metadata[:,0], cmap='rainbow')
...
def biplots(prj,prj2=None,
            n=1,
            plottype='hexbin',plottype2='hexbin',
            nbins=10,
            figsize=-1,
            c=None,c2=None,
            cmap=None,c2map=None,
            show_histo=False,
            figname='',
            scatter_size=None,
            minortick=0.1,
        ...
        for j in np.arange(ncol):
            if(i<j):
                ax = biplot_axis(n,i,j,gs,fig,labels,
                                 ...
                                 c=c,cmap=cmap,
                                 nbins=nbins,
                                 majortick=majortick,minortick=minortick,linewidth=2.5,
                                 plottype=plottype, scatter_size=scatter_size,
                ...
                if prj2 is not None:
                    ax = biplot_axis(n,i,j,gs,fig,labels,
                                     ...
                                     nbins=nbins,
                                     majortick=majortick,minortick=minortick,linewidth=2.5,
                                     plottype=plottype2, scatter_size=scatter_size,
                                     x_range=x_range2, y_range=y_range2)
    plt.tight_layout()
    plt.show()
    if(figname):
        fig.savefig(figname)
...
def biplot_axis(n,i,j,gs,fig,labels,
                Ax=np.zeros(1),Ay=np.zeros(1),
                c=None,cmap=None,
                nbins=1,
                majortick=0.2,minortick=0.1,
                linewidth=2.5,
...
def plot_roc_curve(mus, Zscore, Zscore_set,
