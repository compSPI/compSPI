...
for angle in np.arange(90, 360, 90):
    print(
        "angle = {} / dataset_extended.shape={}".format(angle, dataset_extended.shape)
    )
    i += 1
    ...
    dataset_extended[:, i : i + 1, :, :] = dataset_tmp[
        :, :, 0 : dataset.shape[2], 0 : dataset.shape[3]
    ]  # rotate(dataset,angle, axes=((2,3)), mode='wrap')
#
